...
namespace Microsoft.CodeAnalysis.Text
{
    ...
    public abstract class SourceText
    {
        ...
        private static string Decode(Stream stream, Encoding encoding, out Encoding actualEncoding)
        {
            ...
            using (var reader = new StreamReader(stream, encoding, detectEncodingFromByteOrderMarks: true,  Math.Min(4096, length)))
            {
                string text = reader.ReadToEnd();
        ...
        /// or a single change encompassing the entire text.
        /// </summary>
        public virtual IList<TextChange> GetTextChanges(SourceText oldText)
        {
            int newPosDelta = 0;
