...
#pragma warning restore CA1200 // Avoid using cref tags with a prefix
    ...
    public abstract partial class SyntaxNode
    {
        ...
        internal SyntaxNode GetWeakRedElement(ref WeakReference slot, int index)
        {
            SyntaxNode value = null;
            if(slot!=null)
            {
                value =(SyntaxNode) slot.Target;
                if(value!=null)
                {
                    return value;
                }
        ...
        // handle a miss
        private SyntaxNode CreateWeakItem(ref WeakReference slot, int index)
        {
            var greenChild = this.Green.GetSlot(index);
            var newNode = greenChild.CreateRed(this.Parent, GetChildPosition(index));
            var newWeakReference = new WeakReference(newNode);

            while (true)
            {
                SyntaxNode previousNode = null;
                WeakReference previousWeakReference = slot;
                if(previousWeakReference!=null)
                {
                    previousNode = (SyntaxNode)previousWeakReference.Target;
