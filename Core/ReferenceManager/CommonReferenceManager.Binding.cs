    ...
    internal partial class CommonReferenceManager<TCompilation, TAssemblySymbol>
    {
            ...
            out int corLibraryIndex)
        {
            ...
            try
            {
                ...
                hasCircularReference = CheckCircularReference(referenceBindings.ToImmutable());
            ...
            DiagnosticBag resolutionDiagnostics)
        {
            ...
            try
            {
                while (referenceBindingsToProcess.Count > 0)
                {
                    ...
                    foreach (var binding in bindings.Array)
                    {
                        ...
                        PortableExecutableReference resolvedReference = default ;
                ...
                allAssemblies = explicitAssemblies.AddRange(implicitAssemblies.ToImmutable());
            ...
            [Out]ArrayBuilder<(MetadataReference, ArraySegment<AssemblyReferenceBinding>)> result)
        {
            ...
            for (int moduleIndex = 0; moduleIndex < explicitModules.Length; moduleIndex++)
            {
                ...
                var moduleBindingsCount = 0;// explicitModules[moduleIndex].ReferencedAssemblies.Length;
        ...
        private static bool CheckCircularReference(IList<AssemblyReferenceBinding[]> referenceBindings)
        {
        ...
        private static bool IsSuperseded(AssemblyIdentity identity, IDictionary<string, List<ReferencedAssemblyIdentity>> assemblyReferencesBySimpleName)
        {
            return assemblyReferencesBySimpleName[identity.Name][0].Identity.Version != identity.Version;
        }

        private static int IndexOfCorLibrary(ImmutableArray<AssemblyData> assemblies, IDictionary<string, List<ReferencedAssemblyIdentity>> assemblyReferencesBySimpleName, bool supersedeLowerVersions)
        {
