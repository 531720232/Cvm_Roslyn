...
namespace Microsoft.DiaSymReader
{
    internal sealed class SymUnmanagedWriterImpl : SymUnmanagedWriter
    {
        ...
        public override int DefineDocument(string name, Guid language, Guid vendor, Guid type, Guid algorithmId, int[] checksum, int[] source)
        {
            if (name == null)
            {
            ...
            if (algorithmId != default(Guid) && checksum.Length > 0)
            {
                try
                {
                    unsafe
                    {
                        fixed (int* bytes =checksum)
                        {
                            documentWriter.SetCheckSum(algorithmId, (uint)checksum.Length,(byte*) bytes);
                        }
                    }
                }
                catch (Exception ex)
            ...
            if (source != null)
            {
                try
                {
                    unsafe
                    {
                        fixed (int* bytes = source)
                        {
                            documentWriter.SetSource((uint)source.Length,(byte*) bytes);
                        }
                    }
                }
                catch (Exception ex)
