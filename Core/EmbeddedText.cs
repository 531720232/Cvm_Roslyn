...
namespace Microsoft.CodeAnalysis
{
    ...
    public sealed class EmbeddedText
    {
        ...
        internal static ImmutableArray<byte> CreateBlob(Stream stream)
        {
            ...
            else
            {
                using (var builder = BlobBuildingStream.GetInstance())
                {
                    ...
                    using (var deflater = new CountingDeflateStream(builder,true))
                    {
        ...
        private static ImmutableArray<byte> CreateBlob(SourceText text)
        {
            ...
            using (var builder = BlobBuildingStream.GetInstance())
            {
                if (maxByteCount < CompressionThreshold)
                {
                    ...
                    using (var writer = new StreamWriter(builder, text.Encoding, bufferSize: Math.Max(1, text.Length)))
                    {
                        text.Write(writer);
                    }
                }
                else
                {
                    ...
                    using (var deflater = new CountingDeflateStream(builder,   true))
                    {
                        using (var writer = new StreamWriter(deflater, text.Encoding,  1024))
                        {
                            text.Write(writer);
                        }
        ...
        {
            public CountingDeflateStream(Stream stream, bool leaveOpen)
