...
namespace System.Reflection.Metadata.Ecma335
{
	public sealed class ControlFlowBuilder
	{
		...
		internal int ExceptionHandlerCount
		{
			get
			{
				ImmutableArray<ExceptionHandlerInfo>.Builder lazyExceptionHandlers = _lazyExceptionHandlers;
				if (lazyExceptionHandlers == null)
				{
					return 0;
				}
				return lazyExceptionHandlers.Count;
			}
		}
		...
		{
			AddExceptionRegion(ExceptionRegionKind.Finally, tryStart, tryEnd, handlerStart, handlerEnd, default(LabelHandle), default(EntityHandle));
		}
		...
		{
			AddExceptionRegion(ExceptionRegionKind.Fault, tryStart, tryEnd, handlerStart, handlerEnd, default(LabelHandle), default(EntityHandle));
		}
		...
		public void AddFilterRegion(LabelHandle tryStart, LabelHandle tryEnd, LabelHandle handlerStart, LabelHandle handlerEnd, LabelHandle filterStart)
		{
			...
			AddExceptionRegion(ExceptionRegionKind.Filter, tryStart, tryEnd, handlerStart, handlerEnd, filterStart, default(EntityHandle));
		}
		...
		internal void CopyCodeAndFixupBranches(BlobBuilder srcBuilder, BlobBuilder dstBuilder)
		{
			...
			foreach (Blob blob in srcBuilder.GetBlobs())
			{
				while (true)
				{
					...
					num++;
					branchInfo = ((num != _branches.Count) ? _branches[num] : new BranchInfo(2147483647, default(LabelHandle), ILOpCode.Nop));
		...
		internal void SerializeExceptionTable(BlobBuilder builder)
		{
			if (_lazyExceptionHandlers != null && _lazyExceptionHandlers.Count != 0)
			{
				...
				foreach (ExceptionHandlerInfo lazyExceptionHandler in _lazyExceptionHandlers)
				{
					...
					switch (lazyExceptionHandler.Kind)
					{
					case ExceptionRegionKind.Catch:
						catchTokenOrOffset = MetadataTokens.GetToken(lazyExceptionHandler.CatchType);
						break;
					case ExceptionRegionKind.Filter:
						catchTokenOrOffset = GetLabelOffsetChecked(lazyExceptionHandler.FilterStart);
						break;
					default:
