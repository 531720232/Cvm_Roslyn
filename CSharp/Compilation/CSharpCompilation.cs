...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    public sealed partial class CSharpCompilation : Compilation
    {
        ...
        internal new NamedTypeSymbol GetSpecialType(SpecialType specialType)
        {
            ...
            {
                result = Symbols.Metadata.PE.PEAssemblySymbol.Inst.GetSpecialType(specialType);
               // result = Assembly.CorLibrary.GetSpecialType(specialType);
            }
            ...
            if (HostObjectType != null && (object)_lazyHostObjectTypeSymbol == null)
            {
                ...
                if ((object)symbol == null)
                {
                    ...
                    symbol = new MissingMetadataTypeSymbol.TopLevelWithCustomErrorInfo(
                        new MissingAssemblySymbol(AssemblyIdentity.FromAssemblyDefinition(HostObjectType.Assembly)).Modules[0],
                        ref mdName,
        ...
        private MethodSymbol FindEntryPoint(CancellationToken cancellationToken, out ImmutableArray<Diagnostic> sealedDiagnostics)
        {
            ...
            try
            {
                ...
                else if (viableEntryPoints.Count > 1)
                {
                    ...
                    //     args:new object[0],
                    //     symbols: viableEntryPoints.ToImmutable(),
                    //     additionalLocations: viableEntryPoints.Select(m => m.Locations.First()).OfType<Location>().AsImmutable());

                    var info = new CSDiagnosticInfo(ErrorCode.ERR_MultipleEntryPoints);
        ...
        // to be recycled.
        private WeakReference[] _binderFactories;
        ...
        internal BinderFactory GetBinderFactory(SyntaxTree syntaxTree)
        {
            ...
            previousFactory = (BinderFactory)previousWeakReference?.Target;
            if (previousWeakReference != null && previousFactory!=null)
            {
        ...
        private BinderFactory AddNewFactory(SyntaxTree syntaxTree, ref WeakReference slot)
        {
            var newFactory = new BinderFactory(this, syntaxTree);
            var newWeakReference = new WeakReference(newFactory);

            while (true)
            {
                BinderFactory previousFactory;
                WeakReference previousWeakReference = slot;
                previousFactory = (BinderFactory)previousWeakReference?.Target;
                if (previousWeakReference != null && previousFactory!=null)
                {
                    return previousFactory;
