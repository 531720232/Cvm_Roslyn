...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal abstract class AssemblySymbol : Symbol, IAssemblySymbolInternal
    {
        ...
        internal AssemblySymbol CorLibrary
        {
            get
            {
                return Metadata.PE.PEAssemblySymbol.Inst; // return _corLibrary;
            }
        }
        ...
        internal TypeSymbol GetTypeByReflectionType(Type type, bool includeReferences)
        {
            ...
            if (type.IsArray)
            {
                TypeSymbol symbol = GetTypeByReflectionType(type.GetElementType(), includeReferences);
            ...
            else if (type.DeclaringType != null)
            {
                ...
                var currentTypeInfo = type.IsGenericType ? type.GetGenericTypeDefinition() : type;
                ...
                while (--i >= 0)
                {
                    int forcedArity = nestedTypes[i].GetGenericArguments().Length - nestedTypes[i + 1].GetGenericArguments().Length;
            ...
            {
                AssemblyIdentity assemblyId = AssemblyIdentity.FromAssemblyDefinition(type.Assembly);

                MetadataTypeName mdName = MetadataTypeName.FromNamespaceAndTypeName(
                    type.Namespace ?? string.Empty,
                    type.Name,
                    forcedArity: type.GetGenericArguments().Length);
