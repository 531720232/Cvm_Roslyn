    ...
    public partial class CSharpCompilation
    {
        ...
        internal sealed class ReferenceManager : CommonReferenceManager<CSharpCompilation, AssemblySymbol>
        {
            ...
            private bool CreateAndSetSourceAssemblyFullBind(CSharpCompilation compilation)
            {
                ...
                try
                {
                    IDictionary<(string, string), MetadataReference> boundReferenceDirectiveMap= new Dictionary<(string, string), MetadataReference>() ;
                    ImmutableArray<MetadataReference> boundReferenceDirectives = ImmutableArray<MetadataReference>.Empty;
                    ImmutableArray<AssemblyData> referencedAssemblies = ImmutableArray<AssemblyData>.Empty ;
                    ImmutableArray<PEModule> modules=ImmutableArray<PEModule>.Empty; // To make sure the modules are not collected ahead of time.
                    ImmutableArray<MetadataReference> explicitReferences = ImmutableArray<MetadataReference>.Empty;
