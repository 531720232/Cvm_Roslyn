...
namespace Microsoft.CodeAnalysis.CSharp.Symbols.Metadata.PE
{
        ...
        : NamespaceSymbol
    {
        ...
        /// full name.
        /// </summary>
        private Dictionary<string, Type> _lazyNoPiaLocalTypes;
        ...
        /// </summary>
        private void LazyInitializeTypes(IEnumerable<IGrouping<string, Type>> typeGroups)
        {
            if (this.lazyTypes == null)
            {
                ...
                var skipCheckForPiaType = true;// !moduleSymbol.Module.ContainsNoPiaLocalTypes();
                Dictionary<string, Type> noPiaLocalTypes = null;
                ...
                foreach (var g in typeGroups)
                {
                    foreach (var t in g)
                    {
                        ...
                        //        {
                        //            noPiaLocalTypes = new Dictionary<string, Type>(StringOrdinalComparer.Instance);
                        //        }
        ...
        internal NamedTypeSymbol LookupMetadataType(ref MetadataTypeName emittedTypeName, out bool isNoPiaLocalType)
        {
            ...
            if (result is MissingMetadataTypeSymbol)
            {
                ...
                {
                    result = (NamedTypeSymbol)new MetadataDecoder(ContainingPEModule).GetType1(typeDef);
