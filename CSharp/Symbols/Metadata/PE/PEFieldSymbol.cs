    ...
    internal sealed class PEFieldSymbol : FieldSymbol
    {
        ...
        internal override UnmanagedType MarshallingType
        {
            get
            {
                ...
                return 0;//_containingType.ContainingPEModule.Module.GetMarshallingType(_handle);
            }
        }
        ...
        internal override ImmutableArray<byte> MarshallingDescriptor
        {
            get
            {
                ...
                return default(ImmutableArray<byte>);// _containingType.ContainingPEModule.Module.GetMarshallingDescriptor(_handle);
            }
        }
        ...
        internal override int? TypeLayoutOffset
        {
            get
            {
                ...
                return 0;// _containingType.ContainingPEModule.Module.GetFieldOffset(_handle);
            }
        }
        ...
        private void EnsureSignatureIsLoaded()
        {
            if (_lazyType.IsNull)
            {
                ...
                TypeSymbol typeSymbol = (new MetadataDecoder(moduleSymbol, _containingType)).GetType1(_handle.FieldType);
                ImmutableArray<CustomModifier> customModifiersArray = ImmutableArray<CustomModifier>.Empty;// CSharpCustomModifier.Convert(customModifiers);
                ...
                type = NullableTypeDecoder.TransformType(type, _handle.FieldType, moduleSymbol);
                type = TupleTypeDecoder.DecodeTupleTypesIfApplicable(type, _handle, moduleSymbol);

                _lazyIsVolatile = false;// isVolatile;
            ...
            try
            {
                ...
                foreach (var obj in objs)
                {
                    if (obj is System.Runtime.CompilerServices.FixedBufferAttribute fix)
                    {
                        ...
                        var item = new MetadataDecoder(this.ContainingPEModule).GetType1(fix.ElementType);//; containingAotModule.TypeHandleToTypeMap[fix.ElementType];
        ...
        internal override IEnumerable<CSharpAttributeData> GetCustomAttributesToEmit(PEModuleBuilder moduleBuilder)
        {
            ...
            if (FilterOutDecimalConstantAttribute())
            {
                ...
                foreach(var v in _handle.GetCustomAttributesData())
                {
                    if (v.Constructor.DeclaringType == typeof(System.Runtime.CompilerServices.DecimalConstantAttribute))
                    {
