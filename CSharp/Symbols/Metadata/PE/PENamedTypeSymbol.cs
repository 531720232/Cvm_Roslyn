...
namespace Microsoft.CodeAnalysis.CSharp.Symbols.Metadata.PE
{
    ...
    internal abstract class PENamedTypeSymbol : NamedTypeSymbol
    {
        ...
        {
            var gen = handle.GetGenericArguments();
            ushort arity=(ushort) gen.Length;
        ...
        internal override bool HasCodeAnalysisEmbeddedAttribute
        {
            get
            {
                ...
                return false;// uncommon.lazyHasEmbeddedAttribute.Value();
            }
        }
            ...
            if (!_flags.IsInterface())
            {
                try
                {
                    ...
                    if (token!=null)
                    {
                        ...
                        //                                                                      _handle,
                        //    
                        return (NamedTypeSymbol) decodedType;//moduleSymbol);
                    }
                }
        ...
        public override bool IsValueType => _handle.IsValueType;
        private ImmutableArray<NamedTypeSymbol> MakeDeclaredInterfaces()
        {
            try
            {
                ...
                var interfaceImpls = _handle.GetInterfaces();// moduleSymbol.Module.GetInterfaceImplementationsOrThrow(_handle);
        ...
        internal override IEnumerable<MethodSymbol> GetMethodsToEmit()
        {
            ...
            else
            {
                ...
                var methodDefs = ArrayBuilder<MethodBase>.GetInstance();
                ...
                foreach (var methodDef in methodDefs)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            gapSize = ModuleExtensions.GetVTableGapSize((methodDef).Name);
                        }
                        catch (BadImageFormatException)
                        {
                            gapSize = 1;
        ...
        //    {
        //        return (((PENamedTypeSymbol)x).Handle)==((PENamedTypeSymbol)y).Handle);
        //    }
        //}
                ...
                try
                {
                    foreach (var fieldDef in _handle.GetFields())
                    {
                        ...
                        if ((fieldFlags & FieldAttributes.Static) == 0)
                        {
                            ...
                            var meta = new MetadataDecoder(this.ContainingPEModule, this);
                            TypeSymbol type = meta.GetType1(fieldDef.FieldType);
        ...
        private void LoadMembers()
        {
            ...
            if (_lazyMembersInDeclarationOrder.IsDefault)
            {
                ...
                else
                {
                    ...
                    PooledDictionary<MethodBase, PEMethodSymbol> methodHandleToSymbol = this.CreateMethods(nonFieldMembers);
                ...
                if (!uncommon.lazyContainsExtensionMethods.HasValue())
                {
                    ...
                    switch (this.TypeKind)
                    {
                        ...
                        case TypeKind.Delegate:
                            var moduleSymbol = this.ContainingPEModule;
                            var module = moduleSymbol.Module;
                            bool moduleHasExtension = _handle.IsDefined(typeof(System.Runtime.CompilerServices.ExtensionAttribute),false);// module.HasExtensionAttribute(_handle, ignoreCase: false);
        ...
        private IEnumerable<PENamedTypeSymbol> CreateNestedTypes()
        {
            ...
            try
            {
                nestedTypeDefs =(_handle).GetNestedTypes().ToImmutableArrayOrEmpty();
            }
            catch (BadImageFormatException)
            {
                yield break;
        ...
        private PooledDictionary<MethodBase, PEMethodSymbol> CreateMethods(ArrayBuilder<Symbol> members)
        {
            var moduleSymbol = this.ContainingPEModule;
            var module = moduleSymbol.Module;
            var map = PooledDictionary<MethodBase, PEMethodSymbol>.GetInstance();
            ...
            try
            {
                ...
                {
                    var s1 = module.ShouldImportMethod(methodHandle, moduleSymbol.ImportOptions);
                    if (isOrdinaryEmbeddableStruct ||s1)
                    {
        ...
        private void CreateProperties(Dictionary<MethodBase, PEMethodSymbol> methodHandleToSymbol, ArrayBuilder<Symbol> members)
        {
            var moduleSymbol = this.ContainingPEModule;
            var module = moduleSymbol.Module;
            ...
            try
            {
                foreach (var propertyDef in (_handle).GetProperties())
                {
                    try
                    {
                        ...
                        PEMethodSymbol getMethod = GetAccessorMethod(module, methodHandleToSymbol, propertyDef.GetGetMethod());
                        PEMethodSymbol setMethod = GetAccessorMethod(module, methodHandleToSymbol, propertyDef.GetSetMethod());
        ...
        private void CreateEvents(
            ...
            Dictionary<MethodBase, PEMethodSymbol> methodHandleToSymbol,
            ArrayBuilder<Symbol> members)
        {
            ...
            try
            {
                foreach (var eventRid in (_handle).GetEvents())
                {
                    try
                    {
                        var add = eventRid.GetAddMethod(true);
                        var rem = eventRid.GetRemoveMethod(true);
                        // NOTE: C# ignores all other accessors (most notably, raise/fire).
                        PEMethodSymbol addMethod = GetAccessorMethod(module, methodHandleToSymbol, add);
                        PEMethodSymbol removeMethod = GetAccessorMethod(module, methodHandleToSymbol, rem);
        ...
        private PEMethodSymbol GetAccessorMethod(PEModule module, Dictionary<MethodBase, PEMethodSymbol> methodHandleToSymbol, MethodBase methodDef)
        {
            if (methodDef==null)
            {
                return null;
            ...
            get
            {
                ...
                if (!uncommon.lazyIsByRefLike.HasValue())
                {
                    ...
                    if (this.TypeKind == TypeKind.Struct)
                    {
                        ...
                        isByRefLike = _handle.IsByRef.ToThreeState();
        ...
        internal override NamedTypeSymbol ComImportCoClass
        {
            get
            {
                ...
                if (ReferenceEquals(uncommon.lazyComImportCoClassType, ErrorTypeSymbol.UnknownResultType))
                {
                    ...
                    var cs = Handle.GetCustomAttributes(typeof(System.Runtime.InteropServices.CoClassAttribute),false);

                    foreach(CoClassAttribute v in cs)
                    {
                    //    var me = new MetadataDecoder(this.ContainingPEModule, this);
                    //var t1=    me.GetType(v.GetType());
                    //    //           Interlocked.CompareExchange(ref uncommon.lazyComImportCoClassType, t1, ErrorTypeSymbol.UnknownResultType);
                    //    uncommon.lazyComImportCoClassType = t1;
                    }
        ...
        {
            var us = _handle.GetCustomAttributes(typeof(AttributeUsageAttribute), true)
                ;

            foreach(AttributeUsageAttribute u in us)
            {
                return new AttributeUsageInfo(u.ValidOn, u.AllowMultiple, u.Inherited);
            }
        ...
        {
            private readonly Type[] _genericParameterHandles;
            ...
            internal PENamedTypeSymbolGeneric(
                ...
                : base(moduleSymbol,
                      ...
                      out mangleName)
            {
                Debug.Assert(genericParameterHandles.Length > 0);
                _arity = arity;
            ...
            override internal int MetadataArity
            {
                get
                {
                    return _genericParameterHandles.Length;
                }
            }
            ...
            private void EnsureTypeParametersAreLoaded()
            {
                if (_lazyTypeParameters.IsDefault)
                {
                    ...
                    int firstIndex = _genericParameterHandles.Length - _arity;
