...
namespace Microsoft.CodeAnalysis.CSharp.Symbols.Metadata.PE
{
    ...
    internal sealed class PEMethodSymbol : MethodSymbol
    {
        ...
        private readonly MethodBase _handle;
            ...
            MethodBase methodDef)
        {
            ...
            try
            {
                ...
                MethodImplAttributes implFlags = Handle.GetMethodImplementationFlags();
               _name= methodDef.Name;
                localflags = methodDef.Attributes;
        ...
        public override DllImportData GetDllImportData()
        {
            if (HasFlag(MethodAttributes.PinvokeImpl))
            {
                try
                {
                    var we = Handle.GetCustomAttributes(typeof(System.Runtime.InteropServices.DllImportAttribute), false);
                    var dll = (System.Runtime.InteropServices.DllImportAttribute)we[0];
                    var data = new DllImportData(dll.Value, dll.EntryPoint,MethodImportAttributes.None);
                    return data;
                }
                catch
                {
        ...
        public override int Arity
        {
            get
            {
                ...
                try
                {
                   if(_handle is ConstructorInfo)
                    {
                        return 0;
                    }

                    int typeParameterCount=_handle.GetGenericArguments().Length;
        ...
        internal MethodBase Handle => _handle;
        ...
        internal override int ParameterCount
        {
            get
            {
                ...
                try
                {
                    int parameterCount=_handle.GetParameters().Length;
        ...
        private ImmutableArray<ParameterSymbol> GetPs()
        {
            ...
            var mod = (PEModuleSymbol)this.ContainingModule;
            ...
            foreach (var p in ps)
            {
                var mt = new MetadataDecoder(mod);
                var typ = mt.GetTypeOfToken(p.ParameterType);
                var a1 = PEParameterSymbol.Create(mod, this,
        ...
        PEParameterSymbol _returnTypeParameter;
        private PEParameterSymbol L1()
        {
            var mod = (PEModuleSymbol)this.ContainingModule;
            if (_returnTypeParameter == null)
            {
                if (this.Handle is MethodInfo m)
                {
                    ...
                    pi.IsByRef = false;
                    pi.Type = new MetadataDecoder((mod)).GetType1(rt.ParameterType);
                    _returnTypeParameter = PEParameterSymbol.Create(mod, this,
                        IsMetadataVirtual(), 0, pi, default,
                        true, out bool f);
                }
                if (this.Handle is ConstructorInfo c)
                {
                    var dc = mod.ContainingAssembly.GetSpecialType(SpecialType.System_Void);
                    var pi = new ParamInfo<TypeSymbol>();
                    pi.CustomModifiers = ImmutableArray<ModifierInfo<TypeSymbol>>.Empty;
                    pi.RefCustomModifiers = ImmutableArray<ModifierInfo<TypeSymbol>>.Empty;
                    pi.IsByRef = false;
                    pi.Type = dc;
                    _returnTypeParameter = PEParameterSymbol.Create(mod, this, IsMetadataVirtual(), 0, pi, default, false, out bool a1);
                }
            }
        ...
        internal PEParameterSymbol ReturnTypeParameter => L1();
        ...
        private ImmutableArray<TypeParameterSymbol> GetTs()
        {
            ...
            var ps = Handle.GetGenericArguments();
            int i = 0;
            foreach (var p in ps)
            {
                PETypeParameterSymbol a1 = new PETypeParameterSymbol((PEModuleSymbol)this.ContainingModule, _containingType, (ushort)i, p);
                ls.Add(a1);
                i++;
            }
        ...
        public override ImmutableArray<CSharpAttributeData> GetReturnTypeAttributes() => ImmutableArray<CSharpAttributeData>.Empty;// Signature.ReturnParam.GetAttributes();
        ...
        public override MethodKind MethodKind
        {
            get
            {
                ...
                else if(Handle.Name.Contains("op_"))
                  {
                    k = MethodKind.UserDefinedOperator;
                  }
                else
                {
        ...
        internal override Cci.CallingConvention CallingConvention {
            ...
            get
            {
                var c = Cci.CallingConvention.Standard;

               if(IsExtensionMethod)
                {
                    c |= Cci.CallingConvention.ExplicitThis;
                }
           
               if(GetDllImportData()!=null)
                {
                    c |= Cci.CallingConvention.ThisCall;
                }
               if(TypeArguments.Length>0)
                {
                    c |= Cci.CallingConvention.Generic;
                }
              
                return c;
            }

        }//=> (Cci.CallingConvention)Signature.Header.RawValue;
        ...
        {
            ImmutableArray<string> s = ImmutableArray<string>.Empty;
            var builder = s.ToBuilder();
            var c = _handle.GetCustomAttributes(typeof(ConditionalAttribute), false);

            foreach(ConditionalAttribute cc in c)
            {
                builder.Add(cc.ConditionString);
            }

            return builder.ToImmutableArrayOrEmpty();
